...
export function GET() {
  ...
  const body = {
    ...
    writing_guidance: [
      ...
      "Motion has names. A `motion` entry is { to, wave, cycles, phase }, and the club's loops are: drift (sin ×1), breathe (sin ×2), pulse (sin ×4), swing (tri ×2), sweep (saw ×1), march (saw ×3), blink (square ×4). Prefer one of those shapes over an arbitrary wave/cycle pair — they are what the studio offers by name, and a spec that uses them reads as the same instrument.",
      ...
      "A tile is a layer. `type: \"tiles\"` draws one of the Tiles — a framed square of hand-cut folk ornament, radial and flat-coloured, from the club's third studio. `tile` names one of thirteen (spokes, flames, pinwheel, bloom, union, leaves, orbit, cross, star, rings, alarm, beacon, carnival), `tdensity` is more or less of everything in it, `thand` is how hand-drawn the edges are and `tboil` how many times the hand redraws it over the loop. It brings its own colour, because three flat inks are what a tile is; set the layer's `ink` to \"mix\" to hand it the post's palette instead, or `tpalette` to name one of the tile palettes. It is canvas 2D and periodic in the post's duration, so it loops.",
    ...
    spec: {
      ...
      slides: {
        ...
        fields: {
          ...
          shapes:
            "optional array of up to 6 marks drawn with the type — the club's motifs as placed objects. Each is { kind, x, y, size, weight, rotation, opacity } plus optional ink (hex), under (behind the words), and the deformers: repeat (copies), along ('x'|'y'|'arc'|'ring'), spread, jitter (seeded scatter — set `seed` to reroll), twist (each copy turned further), taper (each copy smaller). kind is circle|oval|square|triangle|line|bar|arc|cross|bracket; weight is the stroke in design units and 0 fills it instead. Every number takes `motion` the same way a layer's does, so a mark can travel over the loop. Marks belong to the sheet, so they survive text:false.",
          count:
